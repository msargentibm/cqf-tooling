...
public class IOUtils {
        ...
        for(String dir : resourceDirectories) {
            ...
            RuntimeResourceDefinition patientDefinition = (RuntimeResourceDefinition)getResourceDefinition(fhirContext, "Patient");
            String patientClassName = patientDefinition.getImplementingClass().getName();
            resources.entrySet().stream()
                    .filter(entry -> entry.getValue() != null)
                    .filter(entry ->  patientClassName.equals(entry.getValue().getClass().getName()))
                    .forEach(entry -> patientPaths.add(entry.getKey()));
        }
    }
        ...
        {
            for (String path : IOUtils.getFilePaths(measureBundleDir, false)) {
                try {
                    resources.put(path, IOUtils.readResource(path, fhirContext, true));
                } catch (Exception e) {
                    //TODO: handle exception
        ...
        resources.entrySet().stream()
                .filter(entry -> entry.getValue() != null)
                .filter(entry ->  bundleClassName.equals(entry.getValue().getClass().getName()))
